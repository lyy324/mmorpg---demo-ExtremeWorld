...
#pragma warning restore 414
		...
		private static readonly Regex s_GeometryTagRegex = new Regex(
			@"</?(?:b|i)>|<(?:color|size|material)=[^<>]*>|</(?:color|size|material)>|<quad(?:\s[^<>]*)?/?>",
...
#pragma warning restore 649
		...
		private bool IsPrefab
		{
			get
			{
				...
				return UnityEditor.PrefabUtility.IsPartOfPrefabAsset(this);
#else
...
#pragma warning restore 219
		...
		/// </summary>
		private int[] BuildGlyphIndexMap(string input)
		{
			int[] indices = new int[input.Length + 1];
			Match tag = this.supportRichText ? s_GeometryTagRegex.Match(input) : Match.Empty;
			int glyphIndex = 0;
			for (int i = 0; i < input.Length;)
			{
				...
				{
					// A quad emits one glyph; formatting tags emit none.
					if (tag.Value.StartsWith("<quad", System.StringComparison.OrdinalIgnoreCase))
					{
						++glyphIndex;
					}
				...
				{
					// TextGenerator omits only these whitespace characters from its vertex buffer.
		...
		private static bool RemapCharacterIndices(HyperTextProcessor.TagCharacterData data, int[] glyphIndices)
		{
			int start = glyphIndices[data.CharacterIndices.StartIndex];
			int end = glyphIndices[data.CharacterIndices.EndIndex + 1];
			if (end <= start)
			{
				return false;
			}
			data.CharacterIndices = new IndexRange(start, end - 1);
			return true;
		}
		...
		private string PostprocessText()
		{
			...
			int[] glyphIndices = BuildGlyphIndexMap(m_TextGeneratorInput);
			// clear existing data
			...
			{
				if (RemapCharacterIndices(s_LinkCharacterData[i], glyphIndices))
				{
					m_LinkGeometryData.Add(new Link(m_LinkGeometryData.Count, s_LinkCharacterData[i], this));
				}
			}
			...
			if (this.TextProcessor.IsRichTextEnabled)
			{
				...
				{
					if (RemapCharacterIndices(s_TagCharacterData[i], glyphIndices))
					{
				...
				if (!this.IsPrefab)
				{
					...
					for (int matchIndex = 0; matchIndex < s_QuadCharacterData.Count; ++matchIndex)
					{
						...
						RemapCharacterIndices(s_QuadCharacterData[matchIndex], glyphIndices);
